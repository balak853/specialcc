...
async def xvvfunc(fullcc, user_id):
...
@Client.on_message(filters.command("cctxt", [".", "/"]))
...
async def stripe_mass_txt_auth_cmd(Client, message):
    ...
    try:
        ...
        try:
            ...
            key = f"chktxt_{message.from_user.id}_{random_text}"
            file_name = f"{key}.txt"
            hitsfile = f"HITS/{file_name}"
            await message.reply_to_message.download(file_name=file_name)
        except:
            resp = """<b>
Gate Name: STRIPE CHARGED 1£ ♻️
CMD: /cctxt

Message: No CC Found in your input ❌

Usage: /cctxt [ in reply to txt file ]
        </b> """
...
- 𝐒𝐭𝐚𝐭𝐮𝐬 - Processing...⌛️    """
        ...
        works = [xvvfunc(i, user_id) for i in ccs]
