...
async def get_charge_resp(result, user_id, fullcc):
    try:
        ...
        if type(result) == str:
            ...
            status   = "𝐀𝐩𝐩𝐫𝐨𝐯𝐞𝐝 ✅"
            response = "Charged 0.1$ 🔥"
            hits     = "YES"
            await forward_resp(fullcc, "Paypal Charge 0.1$", response)
            ...
            "incorrect_cvc" in result.text or
            " CVV2_FAILURE_POSSIBLE_RETRY_WITH_CVV." in result.text
        ...
        elif( '"cvc_check": "pass"' in result.text or
             '"issue":"ORDER_NOT_APPROVED"' in result.text
            ):
            status   = "𝐀𝐩𝐩𝐫𝐨𝐯𝐞𝐝 ✅"
            ...
            "EXISTING_ACCOUNT_RESTRICTED" in result.text
            or "OAS_VALIDATION_ERROR" in result.text
        ):
            status   = "𝐀𝐩𝐩𝐫𝐨𝐯𝐞𝐝 ✅"
            response = "CVV LIVE - EXISTING ACCOUNT RESTRICTED 🟢"
            hits     = "YES"
            await forward_resp(fullcc, "Paypal Charge 0.1$", response)
            ...
            or "PAYMENT_DENIED" in result.text
        ):
            status   = "𝐃𝐞𝐜𝐥𝐢𝐧𝐞𝐝 ❌"
            response = "Card Was Decline"
            hits     = "NO"
        ...
        elif " ACCOUNT_CLOSED. A previously open account is now closed" in result.text:
            status   = "𝐃𝐞𝐜𝐥𝐢𝐧𝐞𝐝 ❌"
            response = "ACCOUNT CLOSED "
            hits     = "NO"

        elif (
            "ACCOUNT_BLOCKED_BY_ISSUER." in result.text
        ):
            status   = "𝐃𝐞𝐜𝐥𝐢𝐧𝐞𝐝 ❌"
            response = "ACCOUNT BLOCKED BY ISSUER"
            hits     = "NO"
